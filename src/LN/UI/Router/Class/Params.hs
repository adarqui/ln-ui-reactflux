module LN.UI.Router.Class.Params (
  Params (..),
  PSRoutingParams,
  emptyParams,
  lookupParam,
...
type Params = M.Map String Param
...
lookupParam p_tag params = M.lookup (show p_tag) params
...
psRoutingParamsToParams ps =
  M.fromList $ catMaybes $ map (\(k, v) -> (paramFromKV' k v)) $ M.toList ps
...
paramTagFromString s =
  case s of
    ...
    "user_name"              ->  Just ParamTag_ByUserName
    "users_names"            ->  Just ParamTag_ByUsersNames
